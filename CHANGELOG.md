...
### Adicionado
- **Aprofundamento das 4 áreas novas (57 lições mais ricas)** — fecha o ponto 1 do roadmap para a *matéria adicionada*: **Educação Física**, **Educação Artística**, **Cidadania** e **O Mundo** deixam de ser lições curtas. Cada lição ganhou **mais secções de exemplos** (blocos `keyvalue`/`steps`/`compare`/`meters`/`stats`, agora todos com leitura em voz alta própria), uma secção **"Um exemplo passo a passo 🔍"** que resolve um caso concreto, e **mais prática**: o questionário de treino passou a ~**4 perguntas** e o teste final a ~**5**. Mantêm-se o tema, o título, o resumo "O que vais aprender", o truque, o facto "Para saberes mais 🌱" e **os `id` dos quizzes** (o progresso não se perde). Conteúdo em pt-PT, com factos reais e identidade açoriana/portuguesa (Açores −1 h, baleia-azul, Paula Rego/Casa das Histórias, fado/Amália, vira do Minho, esfera armilar, os 3 grupos de ilhas…). `pnpm validate`, `pnpm build` e `pnpm typecheck` passam.
